    ...
    public async Task Invoke(FunctionContext context, FunctionExecutionDelegate next)
    {
        ...
        if (!string.IsNullOrWhiteSpace(tokenString))
        {
            try
            {
                ...
                var platform = token.Claims.FirstOrDefault(x => x.Type == AireClaims.Platform)?.Value;
                context.Features.Set(new JwtAuthFeature(principal, token, token.Subject, key!, tokenString, verified, platform));
            }
            catch (Exception ex)
            {
